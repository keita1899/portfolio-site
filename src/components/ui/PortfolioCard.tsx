  ...
  return (
    <Link href={`/portfolios/${portfolio.id}`}>
      <div className="bg-white dark:bg-gray-800 overflow-hidden shadow rounded-lg hover:shadow-lg transition-shadow cursor-pointer">
        {/* サムネイル画像 */}
        {portfolio.thumbnail && (
          <div className="relative h-48 bg-gray-200 dark:bg-gray-700 hover:opacity-95 transition-opacity">
            <Image
        ...
        <div className="p-6">
          ...
          {/* リンク */}
          {(portfolio.url || portfolio.github_url) && (
            <div className="flex gap-2 mb-4">
              {portfolio.url && (
                ...
                >
                  <svg
                    className={`${
                      showEditLink ? 'w-3 h-3 mr-1' : 'w-4 h-4 mr-2'
                    }`}
                    fill="none"
                    stroke="currentColor"
                    viewBox="0 0 24 24"
          ...
          >
            ...
            {showEditLink && (
              <div className="flex justify-end">
                <div
                  onClick={(e) => e.stopPropagation()}
                  className="text-blue-600 hover:text-blue-500 dark:text-blue-400 dark:hover:text-blue-300 text-sm font-medium cursor-pointer"
                >
                  <Link
